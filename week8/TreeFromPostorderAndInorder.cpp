...
public:
    TreeNode* buildTreePostIn(vector<int>& inorder, int is, int ie, vector<int>& postorder, int ps, int pe, map<int, int> &mp){
        ...
        root->left = buildTreePostIn(inorder, is, inRoot-1, postorder, ps, ps + nodesLeft - 1, mp);

        root->right = buildTreePostIn(inorder, inRoot+1, ie, postorder, ps + nodesLeft, pe-1, mp);
